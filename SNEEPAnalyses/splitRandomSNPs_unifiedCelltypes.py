...
if (len(sys.argv) < 2):
	...
	hg38_pos = lift_hg19_to_hg38(converter, randomSNPdir + "/randomSNPs/allRandomSNPs_withAlleleInfo_hg19.txt", randomSNPdir + "/randomSNPs/allRandomSNPs_withAlleleInfo_hg38.txt")

	## get number uniq snps original sneep input per celltype
	number_snps = getNumber(sneep_result + "/snpRegions.bed")
	...
	## mk output dir 
	if not os.path.isdir(os.path.join(randomSNPdir, "randomSNPs/randomSNPs_sneep/")):
		os.mkdir(randomSNPdir + "/randomSNPs/randomSNPs_sneep/")
