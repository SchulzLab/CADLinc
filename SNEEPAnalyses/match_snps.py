...
from helper_general import Outputs, report_mem, start_logger
...
def parse_input_args():
    ...
    parser.add_argument('input_snps_file', action='store', type=str,
                        help="full path of file with one line per input snp formatted chr:pos (e.g 10:12345)")

    parser.add_argument('n_matches', action='store', type=int,
    ...
    parser.add_argument('analysis_name', action='store', type=str,
                        help="description for analysis")


    parser.add_argument('output_root', action='store', type=str,
                        help="Will create a new output directory inside of this specified directory. Therefore it must exists...")
    ...
    return input_snps_file, n_matches, ld_buddies_r2, db_file, analysis_name, output_root,
...
def set_up_outputs(OutputObj):
    ...
    OutputObj.add_output('quality_score_file', 'match_quality_per_snp.tsv', add_root=True)
...
def get_properties_of_index_snp(snp_to_match, anno_df):
    ...
    index_maf, index_gene_count, index_dist_nearest_gene, index_friends_ld = anno_df.loc[anno_df['snpID']==snp_to_match, anno_cols].values[0].tolist()

    return index_maf, index_gene_count, index_dist_nearest_gene, index_friends_ld


def set_thresholds(max_dev_maf=0.05, max_dev_gene_count=0.50, max_dev_dist_nearest_gene=0.50, max_dev_friends_ld=0.50):
    ...
    intervals_maf = np.arange(max_dev_maf/5, max_dev_maf+max_dev_maf/5, max_dev_maf/5)
    intervals_percent_gene_count = np.arange(max_dev_gene_count/5, max_dev_gene_count+max_dev_gene_count/5, max_dev_gene_count/5)
    intervals_percent_dist_nearest_gene = np.arange(max_dev_dist_nearest_gene/5, max_dev_dist_nearest_gene+max_dev_dist_nearest_gene/5, max_dev_dist_nearest_gene/5)
    intervals_percent_friends_ld = np.arange(max_dev_friends_ld/5,max_dev_friends_ld+max_dev_friends_ld/5, max_dev_friends_ld/5)

    return [intervals_maf, intervals_percent_gene_count, intervals_percent_dist_nearest_gene, intervals_percent_friends_ld]


def get_thresholds(index_maf,  intervals_maf, index_gene_count, intervals_percent_gene_count,  index_dist_nearest_gene, intervals_percent_dist_nearest_gene, index_friends_ld, intervals_percent_friends_ld, attempt):

    maf_thresh = (index_maf - intervals_maf[attempt], index_maf + intervals_maf[attempt])


    gene_count_thresh = (np.floor(index_gene_count - index_gene_count*intervals_percent_gene_count[attempt]),
                        np.ceil(index_gene_count + index_gene_count*intervals_percent_gene_count[attempt]))

    dist_nearest_gene_thresh = (np.floor(index_dist_nearest_gene - index_dist_nearest_gene*intervals_percent_dist_nearest_gene[attempt]),
                                np.ceil(index_dist_nearest_gene + index_dist_nearest_gene*intervals_percent_dist_nearest_gene[attempt]))

    friends_ld_thresh = (np.floor(index_friends_ld - index_friends_ld*intervals_percent_friends_ld[attempt]),
                                np.ceil(index_friends_ld + index_friends_ld*intervals_percent_friends_ld[attempt]))
...
def write_excluded_snps(snps_to_match, keep_anno_df, excluded_snps_file):
    ...
    hla_snps_to_exclude = set([x for x in snps_to_match if ( int(x.split(":")[0])==6 ) and (25000000 < int(x.split(":")[1]) < 35000000 )])
...
def write_match_quality(snps_to_match_ordered, matched_snps, n_matches, num_matched_found, quality_score_file,fsummary):
    ...
    insuff_matches_percent = (match_qual_df.insuff_matches.sum()/match_qual_df.shape[0])*100
    if insuff_matches_percent == 0:
        median_match_size_percent = 0
    else:
        median_match_size_percent = (match_qual_df.loc[match_qual_df['insuff_matches']].num_uniq_matched.median()/n_matches)*100
...
def match_snps(input_snps_file, n_matches, ld_buddies_r2, db_file, analysis_name, output_root):
    start = time.time()


    output_dir = os.path.join(output_root, f'{analysis_name}_matched_snps_for_input_snps')
    ...
    # start explicit logger...
    logfile=os.path.join(output_dir,f"{analysis_name}.log")
    ...
    smaller_anno_df.snp_maf = pd.to_numeric(smaller_anno_df.snp_maf, downcast='float', errors='coerce')
    smaller_anno_df.gene_count = pd.to_numeric(smaller_anno_df.gene_count, downcast='integer', errors='coerce')
    smaller_anno_df.dist_nearest_gene = pd.to_numeric(smaller_anno_df.dist_nearest_gene, downcast='float', errors='coerce')
    smaller_anno_df.ld_buddies = pd.to_numeric(smaller_anno_df.ld_buddies, downcast='float', errors='coerce')

    # remove +inf and -inf
    smaller_anno_df.replace([np.inf, -np.inf], np.nan, inplace=True)
    #keep_anno_df = smaller_anno_df[~smaller_anno_df.isna().any(1)].copy()
    keep_anno_df = smaller_anno_df[~smaller_anno_df.isna().any(axis = 1)].copy()
    del smaller_anno_df
    ...
    pool.close()
    pool.join()
    logger.info("Done matching SNPS. Took {:.2f} minutes.".format( (time.time() - mstart)/60))
...
if __name__ == '__main__':
    ...
    input_snps_file, n_matches, ld_buddies_r2, db_file, analysis_name, output_root = parse_input_args()




    _ = match_snps(input_snps_file, n_matches, ld_buddies_r2, db_file, analysis_name, output_root)
